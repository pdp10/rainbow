...
/**
 * This version is preemptive on the level of ready queue.
 * A process of a lower index of queue is more important than another one with
 * a greater intex. A running process which was inside a queue i-th will be preemptive
 * by a process of a queue j-th if j < i.
 *
...
public class PreemptiveMF extends MF implements Preemptive {
    ...
    public PCB minor(PCB ready, PCB running) {
        if (readyCNT.get(running.getSimulatedProcess().getId()).intValue() >
                readyCNT.get(ready.getSimulatedProcess().getId()).intValue()) {
            // preemption
            return running;
        }
        return ready;
     ...
     */
     public void insert(PCB ready) {
        ...
        if(readyCNT.get(ready.getSimulatedProcess().getId()) == null) {
            // ready is just actived. This is the first time that it becomes ready.
	    ...
	    // It adds ready in the ready queue of the correct level. (It was setted at start)
            setNextIndex(ready);
        }
        // TEST PREEMPTION
    ...
    /**
     * It sets the next index of queue for the ready process.
     * This auxiliar procedure will be called by insert.
     *
