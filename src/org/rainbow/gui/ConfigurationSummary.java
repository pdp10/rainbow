...
public class ConfigurationSummary extends JDialog {
    ...
    private JPanel getProcessPanel() {
        if (processPanel == null) {
            ...
            if (timeslice != -1)
                header = header.concat(", " + Language.getTimeSlice() + ": " + timeslice + "):");
            else
        ...
        if (resourceTable == null) {
            ...
            for (int i = 0; i < resource.size(); i++) {
                ...
                if (resourceTemp instanceof PreemptiveResource) {
                    ...
                    newRow.add((new Integer(resourceTemp.getMultiplicity()))
                    .toString());
                    newRow.add((new Integer(((NoPreemptiveResource)resourceTemp).getCeilingPriority())).toString());
                }
                values.add(newRow);
            }
