	...
	private class JPanelGraph extends JPanel {
		...
		private void drawResources(Graphics2D g2) {
			...
			if (maxResStringSize > currMaxResString.length())
				resXEdge = (int) (new TextLayout(currMaxResString,
						this.getFont(), g2.getFontRenderContext()))
						.getAdvance();
			else
				resXEdge = (int) (new TextLayout(currMaxResString.substring(0,
						maxResStringSize - 2) + "..", this.getFont(),
						g2.getFontRenderContext())).getAdvance();
			resXEdge += 3 * (int) (new TextLayout("O", this.getFont(),
					g2.getFontRenderContext())).getAdvance();

			int maxYResEdge = 5 * (int) (new TextLayout("O", this.getFont(),
			...
			for (int i = 1; i <= resources.size(); i++) {
				...
				String mult = "M: "
						+ new Integer(resources.get(i - 1).getMultiplicity())
								.toString();
				label = new TextLayout(mult, this.getFont(),
						g2.getFontRenderContext());
		...
		private void drawProcesses(Graphics2D g2) {
			...
			if (maxProcStringSize > currMaxProcString.length())
				procXEdge = (int) (new TextLayout(currMaxProcString,
						this.getFont(), g2.getFontRenderContext()))
						.getAdvance();
			else
				procXEdge = (int) (new TextLayout(currMaxProcString.substring(
						0, maxProcStringSize - 2) + "..", this.getFont(),
						g2.getFontRenderContext())).getAdvance();
			procXEdge += 3 * (int) (new TextLayout("O", this.getFont(),
					g2.getFontRenderContext())).getAdvance();
					...
					if (blockedList != null) {
						...
						for (int j = 0; j < blockedList.getProcessList().size(); j++) {
							...
							if (!processes.contains(blockedList
								...
								processes.add(blockedList.getProcessList().get(
										j));
							...
							for (int y = 0; y < resources.size()
									&& resPos == -1; y++)
								if (resources.get(y).getName()
										.compareTo(resName) == 0)
									resPos = y;

							for (int y = 0; y < processes.size()
									&& procPos == -1; y++)
								if (processes.get(y).getName()
										.compareTo(procName) == 0)
									procPos = y;
					...
					if (attribution != null) {
						...
						for (int j = 0; j < attribution.getProcessList().size(); j++) {
							...
							if (!processes.contains(attribution
									.getProcessList().get(j)))
								processes.add(attribution.getProcessList().get(
										j));
